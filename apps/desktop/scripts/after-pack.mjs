 ...
 *
 * Stamps the Quorum icon + identity onto the packed Windows Quorum.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Quorum')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Quorum'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Quorum.exe keeps the stock Electron icon`)
  }
}
