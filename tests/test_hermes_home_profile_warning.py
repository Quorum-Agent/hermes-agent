...
profile is active, get_hermes_home() should:
  1. STILL return ~/.quorum (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        profile_dir = tmp_path / ".quorum" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".quorum" / "active_profile").write_text("coder\n")
