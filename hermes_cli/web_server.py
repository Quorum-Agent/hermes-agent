...
app = FastAPI(title="Quorum", version=__version__, lifespan=_lifespan)
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Quorum").strip() or "Quorum"
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Quorum exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  {PRODUCT_NAME} backend listening on {host}:{actual_port}")
            else:
                print(f"  {PRODUCT_NAME} Web UI → http://{host}:{actual_port}")
