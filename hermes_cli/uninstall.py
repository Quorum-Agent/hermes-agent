    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Quorum" comment and following line
                if '# Quorum' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
#      like ``%LOCALAPPDATA%\quorum\git\cmd``, ``%LOCALAPPDATA%\quorum\git\bin``,
#      ``%LOCALAPPDATA%\quorum\git\usr\bin``, ``%LOCALAPPDATA%\quorum\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\quorum\git\`` and Node to
...
def run_gui_uninstall(args):
    ...
    print(color(f"│         ⚕ {PRODUCT_NAME} Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print(f"No {PRODUCT_NAME} Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color(f"This removes the Chat GUI only. The {PRODUCT_NAME} agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The {PRODUCT_NAME} agent at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print(f"The {PRODUCT_NAME} agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Quorum Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color(f"⚠️  WARNING: This will permanently delete ALL {PRODUCT_NAME} data!", Colors.RED, Colors.BOLD))
        ...
        print(f"This will remove the {PRODUCT_NAME} code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print(f"  • {PRODUCT_NAME} PATH entries from shell configs / Windows User PATH")
    print(f"  • {PRODUCT_NAME} wrapper scripts and {PRODUCT_NAME}-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • {PRODUCT_NAME} config/data: {hermes_home}")
        ...
        print(f"  • Keep {PRODUCT_NAME} config/data: {hermes_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    if _is_windows():
        ...
        # like C:\Users\<u>\AppData\Local\quorum\git\cmd, not %LOCALAPPDATA%.
        ...
        if removed_path_entries:
            ...
            log_info(f"No {PRODUCT_NAME}-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info(f"No {PRODUCT_NAME}-set User env vars to remove")
    ...
    log_info(f"Removing {PRODUCT_NAME}-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info(f"No {PRODUCT_NAME}-managed node/npm/npx symlinks found")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/Quorum-Agent/hermes-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/Quorum-Agent/hermes-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Quorum! ⚕")
    print()
