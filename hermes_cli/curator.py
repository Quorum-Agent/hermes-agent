...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/{HOME_DIR_NAME}/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    print(
        f"\nThis will replace the current ~/{HOME_DIR_NAME}/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help=f"Take a manual tar.gz snapshot of ~/{HOME_DIR_NAME}/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help=f"Restore ~/{HOME_DIR_NAME}/skills/ from a curator snapshot "
