...
TIPS = [
    ...
    f"Checkpoints use shadow repos in ~/{HOME_DIR_NAME}/checkpoints/ — your project's .git is never touched.",
    ...
    f"Cron scripts live in ~/{HOME_DIR_NAME}/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    f"The skills audit log at ~/{HOME_DIR_NAME}/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    f"Every interrupt during an agent run is logged to ~/{HOME_DIR_NAME}/interrupt_debug.log with timestamps.",
    ...
    f"MCP ${{ENV_VAR}} placeholders in config are resolved at server spawn — including vars from ~/{HOME_DIR_NAME}/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    f"The skills quarantine at ~/{HOME_DIR_NAME}/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    f'/reload reloads ~/{HOME_DIR_NAME}/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    f'Gateway hooks live under ~/{HOME_DIR_NAME}/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    f'Drop a ~/{HOME_DIR_NAME}/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    f'Drop a YAML in ~/{HOME_DIR_NAME}/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    f'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/{HOME_DIR_NAME}/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    f'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/{HOME_DIR_NAME}/cache/piper-voices/.',
    ...
    f'/reload-skills re-scans ~/{HOME_DIR_NAME}/skills/ so drop-in skills appear without restarting the session.',
    ...
    f'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/{HOME_DIR_NAME}/dashboard-plugins/.',
]
