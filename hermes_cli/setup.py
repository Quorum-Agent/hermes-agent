"""
Interactive setup wizard for Quorum.
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning(f"No inference provider is configured — {PRODUCT_NAME} cannot chat yet.")
...
def setup_terminal_backend(config: dict):
    ...
    print_info(f"Choose where {PRODUCT_NAME} runs shell commands and code.")
...
def _setup_bluebubbles():
    ...
    print_info(f"Connects {PRODUCT_NAME} to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info(f"Connect to messaging platforms to chat with {PRODUCT_NAME} from anywhere.")
...
def setup_telemetry(config: dict):
    ...
    print_info(f"Packages stay under this {PRODUCT_NAME} profile and are not uploaded.")
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color(f"  Would overwrite (conflicts with existing {PRODUCT_NAME} config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color(f"  Note: OpenClaw config values may have different semantics in {PRODUCT_NAME}.", Colors.YELLOW))
        print(color(f"  For example, OpenClaw's tool_call_execution: \"auto\" ≠ {PRODUCT_NAME}'s yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info(f"{PRODUCT_NAME} can preview what would be imported before making any changes.")
    print()
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in {PRODUCT_NAME} (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color(f"│     ⚕ {PRODUCT_NAME} Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    print(
        color(
            "│             ⚕ Quorum Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Quorum installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info(f"Connect {PRODUCT_NAME} to messaging apps to chat from anywhere.")
