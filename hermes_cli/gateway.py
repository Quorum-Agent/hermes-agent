...
from product_identity import HOME_DIR_NAME, PRODUCT_NAME, PRODUCT_SLUG
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    ...
                    + [
                        "list-units",
                        f"{PRODUCT_SLUG}-gateway*",
                        "hermes-gateway*",  # prior-edition units, still ours to track
                        "--plain",
                        "--no-legend",
                        "--no-pager",
                    ],
...
_SERVICE_BASE = f"{PRODUCT_SLUG}-gateway"
...
def get_service_name() -> str:
    ...
    Default home returns ``quorum-gateway``.
    Profile ``<home>/profiles/coder`` returns ``quorum-gateway-coder``.
...
# Prior-edition unit names that COLLIDE with a coexisting upstream Hermes's
# CURRENT unit name. Unlike _LEGACY_SERVICE_NAMES (a defunct name Hermes no
# longer uses), a ``hermes-gateway`` unit could belong either to THIS install
# (our own pre-Quorum-rename unit, safe to migrate to ``quorum-gateway``) or to
# a side-by-side Hermes install (must be left alone). So these are only ever
# flagged when _unit_is_ours() confirms the unit was generated by this install.
_PRIOR_EDITION_SERVICE_NAMES: tuple[str, ...] = ("hermes-gateway.service",)


def _path_in_unit(path: str, unit_text: str) -> bool:
    """True if ``path`` occurs in ``unit_text`` ending at a path boundary.

    A plain ``path in unit_text`` test would let a foreign unit that merely has
    our home/checkout as a string *prefix* (``/home/u/.quorum`` vs
    ``/home/u/.quorum-backup``, or ``/opt/quorum`` vs ``/opt/quorum-staging``)
    be mistaken for ours. Require each occurrence to end at the string end, a
    path separator, or a quote/whitespace/colon that terminates the value in a
    systemd ``Key=Value`` line.
    """
    boundary = ("", "/", "\\", '"', "'", " ", "\t", "\r", "\n", ":", ";")
    start = 0
    while True:
        i = unit_text.find(path, start)
        if i < 0:
            return False
        if unit_text[i + len(path):i + len(path) + 1] in boundary:
            return True
        start = i + 1


def _unit_is_ours(unit_text: str) -> bool:
    """Return True when a gateway unit file was generated by THIS install.

    Used to decide whether a ``hermes-gateway`` unit is our own pre-rename unit
    (safe to migrate) versus a side-by-side upstream Hermes install's unit
    (must never be touched). Ownership signals:

    * the ``X-Quorum-Managed`` marker this edition writes into new units;
    * this install's home (``get_hermes_home()``) or checkout root
      (``PROJECT_ROOT``) appearing in the unit — a unit we generate embeds them
      in ``Environment="HERMES_HOME=..."`` / ``ExecStart`` / ``WorkingDirectory``.

    A coexisting Hermes references a different home and checkout, so none match.
    Conservative by design: on any doubt it returns False and the unit is left
    alone.
    """
    if "X-Quorum-Managed" in unit_text:
        return True
    candidates: list[str] = []
    for src in (get_hermes_home, lambda: PROJECT_ROOT):
        try:
            raw = str(src()).strip()
            if not raw or raw == ".":
                continue
            resolved = os.path.realpath(raw)
            # Reject the filesystem root and implausibly shallow paths (bare
            # ``/``, ``/root``, ``/app``): too generic to safely discriminate
            # ownership by path, and ``realpath('')`` silently yields cwd.
            if not resolved or resolved == os.sep:
                continue
            if len([p for p in resolved.split(os.sep) if p]) < 2:
                continue
            candidates.append(resolved)
        except Exception:
            continue
    return any(_path_in_unit(c, unit_text) for c in candidates)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    ...
    * Results are returned purely for caller inspection; this function
    ...
    for is_system, base in _legacy_unit_search_paths():
        ...
        # Prior-edition names (``hermes-gateway``) collide with a coexisting
        # Hermes's CURRENT unit name, so require an OWNERSHIP check in addition
        # to the ExecStart marker — we migrate our own pre-rename unit but never
        # touch a side-by-side Hermes install's gateway.
        for name in _PRIOR_EDITION_SERVICE_NAMES:
            unit_path = base / name
            try:
                if not unit_path.exists():
                    continue
...
def get_launchd_plist_path() -> Path:
    ...
    Default home → ``ai.quorum.gateway.plist``.
    Profile ``<home>/profiles/coder`` → ``ai.quorum.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.{PRODUCT_SLUG}.gateway-{suffix}" if suffix else f"ai.{PRODUCT_SLUG}.gateway"
...
def systemd_install(
    ...
    # before installing the new quorum-gateway.service. If both remain, they
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.{PRODUCT_SLUG}.gateway-{suffix}" if suffix else f"ai.{PRODUCT_SLUG}.gateway"
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": f"Channel ID where {PRODUCT_NAME} delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            f"5. {PRODUCT_NAME} connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            f"4. Enter them below and {PRODUCT_NAME} will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _setup_weixin():
    ...
    print_info(
        f"  3. {PRODUCT_NAME} will store the returned account_id/token in ~/{HOME_DIR_NAME}/.env."
    )
    print_info(
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info(f'    signal-cli link -n "{PRODUCT_NAME}Agent"')
